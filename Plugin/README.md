...
图片预览直接读取文章目录中的原图；正式构建仍由 Astro 压缩图片。Frontmatter 的 `image: "./景色.png"` 用于首页封面，正文请使用标准 Markdown `![景色](./景色.png)`；Obsidian 专用的 `![[景色.png]]` 不属于网站支持的图片语法。更新项目的预览配置后，关闭所有 Blog 预览分栏，再重新打开，以重启本地预览服务；不需要重新安装插件。
